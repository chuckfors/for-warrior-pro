...
import time
import threading
from PIL import ImageGrab, ImageEnhance, Image, ImageTk, ImageFilter
import pytesseract
...
# ---------------------------------------------------------------------------
# Global App Settings without font_size and theme.
default_app_settings = {
    "ocr_refresh_rate": 1.0,      # seconds between Input Box captures
    ...
    "output_write_delay": 0.5,    # delay after sending text before next action
    "restore_mouse": True        # Option to return mouse to its previous position
}
...
# Themes for UI (only used for some colors now)
light_theme = {
    "bg": "#FFFFFF",
    "fg": "#000000",
    "canvas_bg": "#F0F0F0"
...
class DraggableBox:
    def __init__(self, root, x, y, width, height, color, label, group="", monitor=False):
        ...
        # Use saved geometry if available or position it with provided dimensions.
        ...
        def monitor():
            ...
            with mss.mss() as sct:
                while self.monitoring:
                    ...
                    img = remove_white_line_and_arrow(img)
                    # -----------------------------
                    
                    custom_config = '-c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ --psm 7'
                    ocr_result = pytesseract.image_to_string(img, config=custom_config).strip()
                    print(f"OCR captured ({self.top.title()}):", ocr_result)
                    
                    # Filter for uppercase letters, convert to lowercase, and limit to 5 characters
                    filtered_text = ''.join(filter(str.isalpha, ocr_result)).lower()[:5]
                    ...
                    if filtered_text and filtered_text != self.last_text:
                        ...
                        update_filtered_image_label(img)
            self.set_monitoring_title("")
...
# Create the base font after the root window is created.
base_font = font.Font(family="Arial", size=10)
...
def show_disclaimer():
    ...
    disclaimer_text = (
        ...
        "Please note that this software is not 100% accurate. Always confirm that the correct symbol is in "
        "Level 2 before placing any orders.\n\n"
...
step4_text = ("4. Save Your Layout (Optional):\n"
              ...
              "go to the Options menu and select 'Save Window Locations'.")
...
# --- Troubleshooting and Tips Section Header ---
trouble_header = tk.Label(
    scrollable_frame,
...
trouble_header.pack(anchor="w", padx=10, pady=(10, 5))

# --- Troubleshooting and Tips Content ---
troubleshooting_text = (
    "1. Some stock symbols (e.g., VTVT, VT) may not function as intended. "
    "If you observe any additional symbols that seem to be malfunctioning, please report them so we can update our list.\n\n"
    "2. The program activates your Level 2 text box by simulating a mouse click, which allows it to receive input. "
    "After clicking on a symbol in the Day Trade Dash, please pause briefly before moving your mouse to ensure the selection registers properly.\n\n"
    "3. If a box fails to open or appears unresponsive, try selecting 'Delete Saved Window Locations' from the Options menu, then reopen the box.\n\n"
    "4. For best performance, run the program as an administrator. This can help with window focus and improve compatibility with certain trading platforms.\n\n"
    "5. For persistent issues, try restarting the application. If problems continue, please send detailed feedback so we can work on a resolution."
)

trouble_label = tk.Label(
    scrollable_frame,
...
trouble_label.pack(anchor="w", padx=10, pady=(0, 15))
...
def open_advanced_settings():
    ...
    restore_mouse_var = tk.BooleanVar(value=app_settings.get("restore_mouse", False))
    ...
    def reset_to_defaults():
        ...
        restore_mouse_var.set(default_app_settings["restore_mouse"])
    ...
    # New option to restore mouse position after clicking.
    tk.Checkbutton(adv_frame,
                   text="Return Mouse to Previous Position",
                   font=base_font,
                   variable=restore_mouse_var,
                   command=lambda: app_settings.update({"restore_mouse": restore_mouse_var.get()})
                  ).pack(pady=(10, 15))
...
def type_text(text, group):
    ...
    # Record current mouse position if 'restore_mouse' is enabled.
    original_mouse_pos = pyautogui.position() if app_settings.get("restore_mouse", False) else None
    sent = False
    for out in outputs:
    ...
    # Restore the mouse position if the option is enabled.
    if app_settings.get("restore_mouse", False) and original_mouse_pos:
...
def update_filtered_image_label(img):
    img_tk = ImageTk.PhotoImage(img)
    filtered_image_label.config(image=img_tk)
    filtered_image_label.image = img_tk  # Keep a reference to avoid garbage collection

def remove_white_line_and_arrow(img):
    # Convert the PIL image to a NumPy array
    img_array = np.array(img)

    # Apply morphological operations to remove small white elements
    kernel = np.ones((3, 3), np.uint8)
    img_array = cv2.erode(img_array, kernel, iterations=1)  # Erode to remove small elements
    img_array = cv2.dilate(img_array, kernel, iterations=1)  # Restore the text
