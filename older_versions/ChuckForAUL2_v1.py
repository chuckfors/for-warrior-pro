        ...
        def monitor():
            ...
            with mss.mss() as sct:
                while self.monitoring:
                    ...
                    # Filter to only allow uppercase letters A-Z (whitelist) then convert to lowercase.
                    filtered_text = ''.join(filter(lambda x: x in "ABCDEFGHIJKLMNOPQRSTUVWXYZ", ocr_result)).lower()
...
# Note: The feedback bar has been removed.
...
def open_advanced_settings():
    ...
    def save_current_settings():
        app_settings["ocr_refresh_rate"] = ocr_rate_var.get()
        app_settings["output_focus_delay"] = focus_delay_var.get()
        app_settings["output_prewrite_delay"] = prewrite_delay_var.get()
        app_settings["output_write_delay"] = write_delay_var.get()
        app_settings["restore_mouse"] = restore_mouse_var.get()
        save_app_settings()
    ...
    tk.Checkbutton(adv_frame, text="Return Mouse to Previous Position", variable=restore_mouse_var).pack()
