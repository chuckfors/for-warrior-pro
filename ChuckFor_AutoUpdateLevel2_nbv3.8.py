        ...
        def monitor():
            ...
            with mss.mss() as sct:
                while self.monitoring:
                    ...
                    # Filter out non-alphabetic characters (preserving original case)
                    filtered_text = ''.join(filter(str.isalpha, ocr_result))
                    
                    # If the filtered text is more than 6 characters, set it to empty;
                    # Otherwise, limit to the first 5 characters.
...
trouble_header = tk.Label(
    ...
    font=(base_font.actual("family"), 14, "bold"),  # Same size and bold as your instructions title
    bg=light_theme["bg"],
    fg=light_theme["fg"]
)
