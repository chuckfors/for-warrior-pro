...
class DraggableBox:
    def __init__(self, root, x, y, width, height, color, label, group="", monitor=False):
        """
        A draggable, resizable box with persistent geometry.
        The 'group' parameter helps route text between inputs and outputs.
        If color is "clear", it uses a transparent background.
        """
        self.group = group
...
# (For brevity, these functions follow the same pattern.)
def open_a_input():
    global a_input
